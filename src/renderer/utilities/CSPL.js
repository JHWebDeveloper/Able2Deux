...
CSPL._gaussJ.solve = (A, x) => { // in Matrix, out solutions
	const m = A.length
...
CSPL.getNaturalKs = (xs, ys, ks) => {	// in x values, in y values, out k values
	const n = xs.length - 1
	const A = CSPL._gaussJ.zerosMat(n + 1, n + 2)
